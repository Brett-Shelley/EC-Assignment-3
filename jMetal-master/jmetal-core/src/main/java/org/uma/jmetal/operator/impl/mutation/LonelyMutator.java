...
public class LonelyMutator implements MutationOperator<DoubleSolution> {
  ...
  private void doMutation(double probability, DoubleSolution list) {
    int width = 20;
    int height = 20;
    ArrayList<Integer> our_list = new ArrayList<Integer>(Collections.nCopies(width*height, 0));

    for(int i = 0; i < 50; i++){
      our_list.set((int)Math.floor(list.getVariableValue(i)), 1);
    }
    ...
    for(int i = 0; i < width * height; i++){
      neighbours.add(neighbours(our_list, i, width, height));
    ...
    int max_num_of_elements_that_can_be_moved = Math.min(mins.size(), maxes.size());
    ...
    for(int i = 0; i < number_of_elements_we_will_move; i++){
      ...
      Collections.swap(our_list, maxes.get(index_1), mins.get(index_2));
    ...
    for (int i = 0; i < 50; i++) {
      list.setVariableValue(i, (double)last_list.get(i));
    }
  }
  ...
  private int neighbours(ArrayList<Integer> list, int i, int width, int height){
    ...
    if(((ret_val & (1 << 0)) > 0) && (list.get(twoD_to_oneD_coords(new Point(p.x-1,p.y-1), width)) > 0)){total++;}
    if(((ret_val & (1 << 1)) > 0) && (list.get(twoD_to_oneD_coords(new Point(p.x,p.y-1), width)) > 0)){total++;}
    if(((ret_val & (1 << 2)) > 0) && (list.get(twoD_to_oneD_coords(new Point(p.x+1,p.y-1), width)) > 0)){total++;}
    if(((ret_val & (1 << 3)) > 0) && (list.get(twoD_to_oneD_coords(new Point(p.x-1,p.y), width)) > 0)){total++;}
    if(((ret_val & (1 << 4)) > 0) && (list.get(twoD_to_oneD_coords(new Point(p.x+1,p.y), width)) > 0)){total++;}
    if(((ret_val & (1 << 5)) > 0) && (list.get(twoD_to_oneD_coords(new Point(p.x-1,p.y+1), width)) > 0)){total++;}
    if(((ret_val & (1 << 6)) > 0) && (list.get(twoD_to_oneD_coords(new Point(p.x,p.y+1), width)) > 0)){total++;}
    if(((ret_val & (1 << 7)) > 0) && (list.get(twoD_to_oneD_coords(new Point(p.x+1,p.y+1), width)) > 0)){total++;}

    return total;
  }

  private Point oneD_to_twoD_coords(int i, int row_length){
    return new Point(i % row_length, (int)Math.floor(i / row_length));
  }
